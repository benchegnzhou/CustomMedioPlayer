...
public final class DateUtils {
    ...
    /**
     * 获取多长时间前  abort > serverTime
     *
     * @param serverTime
     * @param abort
     * @return
     */
    public static String getOverMinuteString(long serverTime, long abort) {

        long minute = (serverTime - abort) / (1000 * 60);
//        String str = "0分钟前";
        String str = "";
        if (minute >= 0 && minute < 60) {
            ...
            str = minute / (60 * 24) + "天前";
        } else if (minute >= 60 * 24 * 30 && minute < 60 * 24 * 30 * 12) {
            str = minute / (60 * 24 * 30) + "月前";
        } else if (minute >= 60 * 24 * 30 * 12) {
            str = minute / (60 * 24 * 30 * 12) + "年前";
        }
        return str;
    }
     ...
     * @return
     */
    public static String getOverTimeString(long serverTime, long abort) {
...
//        String str = "0分钟前";
        ...
        if (minute < 2) {
            str = "";
        } else if (minute >= 2 && minute < 60) {
            str = minute + "分钟前";
        } else if (minute >= 60 && minute < 24 * 60) {
            str = minute / 60 + "小时前";
        } else {
            return getYYYYMMDDHHmm(abort);
        }
        return str;
    }
    ...
    /**
     * 将一个毫秒值格式化成
     * 2016年12月7日0时49分46秒   的形式
     *
     * @param time
     * @return
     */
    public static String TimeFormat_all(long time) {
     ...
     * @return
     */
    public static String videoTimeFormat(long time) {
        StringBuilder timeBuilder = new StringBuilder();
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeInMillis(time);
        int hour = calendar.get(Calendar.HOUR_OF_DAY)-8;   //暂时不知道原因为是么多8个小时
    ...
    public static String systemTimeFormat() {
        StringBuilder timeBuilder = new StringBuilder();
        Calendar calendar = Calendar.getInstance();
        int hour = calendar.get(Calendar.HOUR_OF_DAY);   //暂时不知道原因为是么多8个小时
        int minute = calendar.get(Calendar.MINUTE);
        int second = calendar.get(Calendar.SECOND);
        if (hour<10) {   //有小时，添加小时
            timeBuilder.append("0");
